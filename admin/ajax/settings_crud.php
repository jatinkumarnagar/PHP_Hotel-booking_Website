...
if (isset($_POST['upd_contacts'])) {
    ...
    $q = "UPDATE `contact_detail` SET `address`=?, `gmap`=?, `pn1`=?, `pn2`=?, `email`=?, `fb`=?, `insta`=?, `tw`=?, `iframe`=? WHERE `sr_no`=?";
    $values = [
        $frm_data['address'],
        $frm_data['gmap'],
        $frm_data['pn1'],
...
if (isset($_POST['add_member'])) {
    ...
    if ($img_r == 'inv_img') {
        echo 'Invalid image mime or format';
    } else if ($img_r == 'inv_size') {
        echo 'Invalid size greater than 2MB';
